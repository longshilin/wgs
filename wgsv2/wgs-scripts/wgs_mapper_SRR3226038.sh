...
$SAMTOOLS/samtools view -Sb - > SRR3226038.bam" >&2
time $BWA/bwa mem -t $THREAD -R $INFO_RG $REF $INPUT_FILE_R1 $INPUT_FILE_R2 | \
$SAMTOOLS/samtools view -Sb - > SRR3226038.bam && echo "*** bwa mapping done" && echo "" >&2

rm -f $INPUT_FILE_R1 $INPUT_FILE_R2

# 步骤二 排序
echo "###COMMAND LINE###：" >&2
echo "$SAMTOOLS/samtools sort -@ $THREAD -m $MEMORY -O bam -o SRR3226038.sorted.bam \
SRR3226038.bam -T PREFIX.bam" >&2
time $SAMTOOLS/samtools sort -@ $THREAD -m $MEMORY -O bam -o SRR3226038.sorted.bam \
SRR3226038.bam -T PREFIX.bam && echo "*** sort bam done" && echo "" >&2

rm -f SRR3226038.bam

# 步骤三 标记重复
echo "###COMMAND LINE###：" >&2
echo "$GATK MarkDuplicates -I SRR3226038.sorted.bam -O SRR3226038.sorted.markdup.bam \
-M SRR3226038.sorted.markup_metrics.txt" >&2
time $GATK MarkDuplicates -I SRR3226038.sorted.bam -O SRR3226038.sorted.markdup.bam \
-M SRR3226038.sorted.markup_metrics.txt 1>&2 && echo "*** 对BAM文件进行重复标记" && echo "" >&2
...
echo "###COMMAND LINE###：" >&2
echo "$GATK HaplotypeCaller -R $REF --emit-ref-confidence GVCF -I SRR3226038.sorted.markdup.bam \
-O SRR3226038.g.vcf" >&2
time $GATK HaplotypeCaller -R $REF --emit-ref-confidence GVCF -I SRR3226038.sorted.markdup.bam \
-O SRR3226038.g.vcf && echo "*** 生成样本SRR3226038的中间变异检测文件gvcf" && echo "" >&2
...
$HADOOP/hadoop fs -put -f SRR3226038.g.vcf /wgsv2/output/gvcf && echo "*** 变异检测结果gvcf文件上传到HDFS上" && echo "">&2
echo "############" `date "+%Y-%m-%d %H:%M:%S"` "############"
echo "--- 结束处理样本 SRR3226038 ---" && echo "" >&2 && echo "" >&2